...
Singleton {
    ...
    function set(entry: var): void {
        ...
        relink.command = ["ln", "-sfn", entry.path, Config.wallpaperLink];
        relink.running = true;
    }
    ...
    Process {
        ...
        command: ["ls", "-1", Config.wallpaperDir]
    ...
    Process {
        ...
        command: ["readlink", "-f", Config.wallpaperLink]
    ...
    function parseListing(text: string): void {
        ...
        root.all = names.map(name => ({
            name: name,
            path: `${Config.wallpaperDir}/${name}`,
            url: root.fileUrl(`${Config.wallpaperDir}/${name}`),
