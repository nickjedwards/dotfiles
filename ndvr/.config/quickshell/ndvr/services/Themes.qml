...
Singleton {
    ...
    // Under XDG_STATE_HOME, not ~/.config. This is state — what you last
    // picked — rather than configuration, and on a stow-managed setup like
    // this one ~/.config/ndvr is a symlink straight into the dotfiles repo,
    // so writing there would dirty a git tree every time anyone changed the
    // theme. FileView creates the parent directory itself.
    //
    // Not in Config either, unlike every other path in this shell: Config's
    // palette reads `current`, so a path read back from Config would have
    // the two singletons constructing each other.
    readonly property string path: {
        const state = Quickshell.env("XDG_STATE_HOME");
        const base = state ? state : `${Quickshell.env("HOME")}/.local/state`;
        return `${base}/ndvr/theme`;
    }
